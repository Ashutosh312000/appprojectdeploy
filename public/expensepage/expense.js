...
window.addEventListener("DOMContentLoaded", async () => {
   ...
   if( localStorage.getItem('per_page')){
     ...
     if(decodeToken.ispremiumuser==true){
      ...
      const response1= await  axios.get(`http://18.117.220.99:3000/expense/getIndex?per_page=${PER_PAGE}`,{headers:{"Authorization" : token}})
            goto(response1);
...
goto = (response) => {
    ...
    gotopage = (pagenumber) => {
        ...
        axios.get(`http://18.117.220.99:3000/expense/getIndex?page=${pagenumber}&per_page=${PER_PAGE}`,{headers:{"Authorization" : token}})
            .then((response) => {
                console.log('hello')
                storeSection.remove();
                pagination.remove();
...
async function expensesubmit(e) {
    try{
        ...
        await axios.post("http://18.117.220.99:3000/expense/postexpense", {expensedetails},{headers:{"Authorization" : token}})
            .then((response) => {
...
deleteuser = function (e,key) {
    ...
    const token=localStorage.getItem('token'); 
    axios.delete(`http://18.117.220.99:3000/expense/deleteAddExpense/${key}`,{headers:{"Authorization" : token}})
        .then((response) => {
...
premiumbtn.onclick= async function(e){   
    ...
    const response=await axios.get('http://18.117.220.99:3000/purchase/premiummembership', {headers:{"Authorization" : token}});
    var options= 
    {
        ...
        "handler": async function(response){
          const response1=  await axios.post('http://18.117.220.99:3000/purchase/updatetransactionstatus',{ 
    ...
    rzp1.on('payment.failed',async function(response){ 
        await axios.post('http://18.117.220.99:3000/purchase/updatetransactionstatusfailed',{ 
            order_id:options.order_id,
            payment_id:response.razorpay_payment_id, 
        },{headers : {"Authorization" :token}})  
...
showleaderboardbtn.onclick= async function(e){  
    if(showleaderboardbtn.value=='off'){
        ...
        const response=await axios.get('http://18.117.220.99:3000/premium/showleaderboard', {headers:{"Authorization" : token}});
